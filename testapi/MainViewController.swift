...
class MainViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func callapi(){
        Task{
        do {
            let result: weatherData = try await weatherRequest().requestData(
                method: .get,
                path: .thirtySixHour,
                parameters: WeatherRequest(Authorization: "CWA-A4AA97C5-E3C1-4BEB-B730-688876F81863", locationName: city)
            )
            weatherDat = result // 儲存返回的數據
            DispatchQueue.main.async {
                self.tbvsee.reloadData() // 更新表格視圖
            }
            print(result)
        } catch {
            print("Error: \(error)")
